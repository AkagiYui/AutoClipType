...
ClipboardTyper 是一个 macOS 菜单栏工具，用来把本机剪贴板里的文本模拟成逐字键盘输入。它适合 VNC、远程桌面等不支持剪贴板共享或剪贴板同步不可靠的场景。
...
ClipboardTyper 纯本地运行。它只会在你主动点击菜单或按下热键时读取当前剪贴板文本，不监听剪贴板变化，不保存剪贴板历史，不上传任何内容，也不需要网络功能。

## 权限

macOS 要求模拟键盘输入的 app 获得辅助功能权限。

首次使用前，请打开：系统设置 > 隐私与安全性 > 辅助功能，然后允许 ClipboardTyper。
...
```sh
xcrun notarytool store-credentials "ClipboardTyper-notary" \
...
PROJECT="ClipboardTyper.xcodeproj"
SCHEME="ClipboardTyper"
TEAM_ID="4M56H5UB5R"
NOTARY_PROFILE="ClipboardTyper-notary"

# 所有中间产物都放在 build/developer-id，方便反复测试时清理。
BUILD_ROOT="$PWD/build/developer-id"
ARCHIVE_PATH="$BUILD_ROOT/ClipboardTyper.xcarchive"
EXPORT_DIR="$BUILD_ROOT/export"
EXPORT_OPTIONS_PLIST="$BUILD_ROOT/ExportOptions.plist"
APP_PATH="$EXPORT_DIR/ClipboardTyper.app"
NOTARY_ZIP="$BUILD_ROOT/ClipboardTyper-for-notary.zip"
FINAL_ZIP="$BUILD_ROOT/ClipboardTyper-notarized.zip"
...
```sh
xcrun notarytool log "submission-id" --keychain-profile "ClipboardTyper-notary"
```

### 打包为 DMG

如果把 `.dmg` 作为最终分发物，建议也对 DMG 提交公证并 stapler。下面的命令会把已经导出的 `build/developer-id/export/ClipboardTyper.app` 打包成 DMG，窗口里包含 ClipboardTyper.app、Applications 快捷方式，以及一张指引用户拖拽安装的背景图。
...
APP="build/developer-id/export/ClipboardTyper.app"
NOTARY_PROFILE="ClipboardTyper-notary"
...
FINAL_DMG="$PWD/build/developer-id/ClipboardTyper-notarized.dmg"
...
rm -f "$FINAL_DMG"
create-dmg \
	--volname "ClipboardTyper" \
	--background "$BACKGROUND" \
	--window-size 640 400 \
	--text-size 12 \
	--icon-size 96 \
	--icon "ClipboardTyper.app" 155 205 \
	--hide-extension "ClipboardTyper.app" \
...
COMPONENT_PKG="$PKG_ROOT/ClipboardTyper-component.pkg"
FINAL_PKG="$PWD/build/developer-id/ClipboardTyper.pkg"
...
# 2. 构造安装根目录。安装时会把 ClipboardTyper.app 放到 /Applications。
rm -rf "$PKG_ROOT"
mkdir -p "$INSTALL_ROOT/Applications"
ditto "$APP" "$INSTALL_ROOT/Applications/ClipboardTyper.app"

# 3. 生成组件包。
pkgbuild \
	--root "$INSTALL_ROOT" \
	--identifier com.akagiyui.ClipboardTyper.pkg \
	--version 1.0 \
	--install-location / \
	"$COMPONENT_PKG"
